...
const actions = {
  async fetchStudentsDetails({ commit }) {
    const response = await fetch("https://www.mini-project.learnlinuxfree.tech/api/students");
  ...
  async addStudent({ commit }, newStudent) {
    const res = await fetch("https://www.mini-project.learnlinuxfree.tech/api/students", {
      method: "POST",
      headers: {
  ...
  async updateStudent({ commit }, updatedStudent) {
    const res = await fetch(`https://www.mini-project.learnlinuxfree.tech/api/students/${updatedStudent.id}`, {
      method: "PUT",
      headers: {
        "Content-type": "application/json",
      },
  ...
  async deleteStudent({ commit }, student) {
    if (confirm("Are you sure?")) {
      const res = await fetch(`https://www.mini-project.learnlinuxfree.tech/api/students/${student.id}`, {
        method: "DELETE",
      });
    }
  },
