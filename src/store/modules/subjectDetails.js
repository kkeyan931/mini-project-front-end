...
const actions = {
  async fetchSubjectsDetails({ commit }) {
    const response = await fetch("https://www.mini-project.learnlinuxfree.tech/api/subjects");
    const data = await response.json();
    commit("setSubjectsDetails", data);
  },
  async addSubject({ commit }, newSubject) {
    const res = await fetch("https://www.mini-project.learnlinuxfree.tech/api/subjects", {
      method: "POST",
      headers: {
  ...
  async updateSubject({ commit }, updatedSubject) {
    const res = await fetch(
      `https://www.mini-project.learnlinuxfree.tech/api/subjects/${updatedSubject.id}`,
      {
        method: "PUT",
        headers: {
          "Content-type": "application/json",
  ...
  async deleteSubject({ commit }, subject) {
    if (confirm("Are you sure?")) {
      const res = await fetch(`https://www.mini-project.learnlinuxfree.tech/api/subjects/${subject.id}`, {
        method: "DELETE",
      });
    }
  },
